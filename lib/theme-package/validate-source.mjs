...
function assertSingleLineText(value, field, maximum) {
...
function assertManifest(manifest, { packaged = false } = {}) {
  ...
  assertSingleLineText(manifest.name, "name", 80);
  ...
  assertSingleLineText(manifest.author.name, "author.name", 80);
  if (manifest.author.url !== undefined) {
    assertSingleLineText(manifest.author.url, "author.url", 2048);
    let authorUrl;
    try {
  ...
  for (const [name, resource] of Object.entries(manifest.resources)) {
    ...
    assertSingleLineText(resource.path, `resources.${name}.path`, 180);
...
function assertTheme(theme, manifest) {
  ...
  assertSingleLineText(theme.name, "theme.name", 80);
  ...
  assertSingleLineText(theme.text.tagline, "theme.text.tagline", 160);
  assertSingleLineText(theme.text.quote, "theme.text.quote", 80);
...
async function readJson(sourceRoot, name) {
  const bytes = await readStableFile(path.join(sourceRoot, name), {
    maximum: MAX_JSON_BYTES,
    invalidCode: "SOURCE_FILE_INVALID",
...
export async function validateSource(sourceDirectory) {
  ...
  const { resources, resourceBytes } = await validateResourceBytes(
    manifest,
    (_name, declaration, maximum) => readStableFile(
  ...
  for (const name of ["LICENSE.txt", "NOTICE.txt"]) {
    if ((await fs.lstat(path.join(sourceRoot, name)).catch(() => null))?.isFile()) {
      optionalEntries.set(name, await readStableFile(path.join(sourceRoot, name), {
        maximum: MAX_JSON_BYTES,
        invalidCode: "SOURCE_FILE_INVALID",
        missingCode: "SOURCE_FILE_MISSING",
        label: name,
...
export function validatePackageDeclarations(entries) {
