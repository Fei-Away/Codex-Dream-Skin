...
export function updateCrc32(value, bytes) {
...
export function parseStrictCentralDirectory(value, {
  baseOffset = 0,
  fileSize = Buffer.byteLength(value),
} = {}) {
  const bytes = Buffer.from(value);
  if (fileSize > MAX_PACKAGE_BYTES) fail("CONTAINER_SIZE_LIMIT", "Package exceeds 32 MiB.");
  if (fileSize < 22 || bytes.length < 22) fail("CONTAINER_END_MISSING", "ZIP end record was not found.");
  const endOffset = findEnd(bytes);
  const absoluteEndOffset = baseOffset + endOffset;
  const commentLength = bytes.readUInt16LE(endOffset + 20);
  if (absoluteEndOffset + 22 + commentLength !== fileSize) {
  ...
  if (
    ...
    || centralOffset < baseOffset
    || centralOffset - baseOffset > bytes.length
  ) {
  ...
  const metadata = [];
  const seen = new Set();
  let cursor = centralOffset - baseOffset;
  let expandedBytes = 0;
  ...
  return { metadata, centralOffset };
}

export function parseStrictLocalHeader(value, entry) {
  const bytes = Buffer.from(value);
  if (bytes.length < 30 || bytes.readUInt32LE(0) !== LOCAL_SIGNATURE) {
    fail("CONTAINER_LOCAL_HEADER_INVALID", `${entry.name} has an invalid local header.`, entry.name);
  }
  const versionNeeded = bytes.readUInt16LE(4);
  ...
  if (versionNeeded > 20 || extraLength !== 0 || nameLength > 180 || bytes.length < 30 + nameLength) {
    fail("CONTAINER_LOCAL_HEADER_INVALID", `${entry.name} has invalid local header bounds.`, entry.name);
  }
  const localName = decodeName(bytes.subarray(30, 30 + nameLength), localFlags);
  if (
    localName !== entry.name || localFlags !== entry.flags || localMethod !== entry.method
...
export function assertStrictEntryRanges(ranges, centralOffset) {
  ranges.sort((left, right) => left.start - right.start);
  if (!ranges.length || ranges[0].start !== 0 || ranges.at(-1).end !== centralOffset) {
...
export function readStrictZip(value) {
  const bytes = Buffer.from(value);
  const { metadata, centralOffset } = parseStrictCentralDirectory(bytes);
  const ranges = [];
  ...
  for (const entry of metadata) {
    ...
    const localHeaderBytes = bytes.subarray(
      entry.localOffset,
      Math.min(centralOffset, entry.localOffset + 30 + 180),
    );
    const dataStart = entry.localOffset + parseStrictLocalHeader(localHeaderBytes, entry);
