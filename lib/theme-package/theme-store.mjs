...
async function assertNoLinkComponents(fileSystem, target) {
  ...
  for (const component of components) {
    ...
    if (entry.isSymbolicLink()) {
      fail("INSTALL_STORE_INVALID", "State root must not contain symbolic links or junctions.");
    }
  }
}
...
export async function installPreparedTheme({
  ...
  await assertNoLinkComponents(fileSystem, requestedRoot);
  ...
  try {
    ...
    if (backupDirectory) {
      try {
        ...
        fail(
          "INSTALL_RECOVERY_REQUIRED",
          "The new theme was installed, but its previous backup could not be removed. Manual recovery is required.",
          null,
          true,
        );
      }
