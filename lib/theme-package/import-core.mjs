...
export function addApplyResult(report, status) {
  const result = { ...report, apply: { status } };
  if (status === "failed-after-install") {
    return {
      ...result,
      pass: false,
...
export function compilePortableTheme(manifest, portableTheme, platform) {
  ...
  return platform === "macos"
    ? { ...common, packageContentHash: manifest.contentHash, colors: { ...portableTheme.palette } }
    : { ...common, packageContentHash: manifest.contentHash, palette: { ...portableTheme.palette } };
}
...
export async function importThemePackage({
  ...
  try {
    ...
    if (expectedContentHash !== null && report.contentHash !== expectedContentHash) {
      fail(
        "PACKAGE_CONFIRMATION_STALE",
        "The theme package changed after it was reviewed. Inspect it again before installing.",
      );
    }
