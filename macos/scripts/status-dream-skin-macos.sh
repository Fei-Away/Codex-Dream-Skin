...
if [ -f "$AUTOLOAD_STATE_PATH" ]; then
  AUTOLOAD_SNAPSHOT="$(/bin/cat "$AUTOLOAD_STATE_PATH" 2>/dev/null)"
  AUTOLOAD_ENABLED="$(read_json_text_field "$AUTOLOAD_SNAPSHOT" enabled)"
  AUTOLOAD_PAUSED="$(read_json_text_field "$AUTOLOAD_SNAPSHOT" paused)"
...
if [ "$JSON" = "true" ]; then
  ...
  printf '{"session":"%s","operation":"%s","operationMessage":"%s","port":%s,"injectorAlive":%s,"cdpOk":%s,"codexRunning":%s,"themeName":"%s","appliedThemeName":"%s","autoLoadEnabled":%s,"autoLoadPaused":%s,"autoLoadAgent":%s}\n' \
