...
canonical_process_started_at() {
  LC_ALL=C TZ=UTC process_started_at "$1"
}
...
hot_reapply_theme() {
  ...
  local operation_token="${3:-}"
  local expected_theme_id="${4:-}"
  local expected_content_hash="${5:-}"
  ...
  if ! "$NODE" "$INJECTOR" --once --port "$port" --theme-dir "$THEME_DIR" \
    --timeout-ms "$timeout_ms" "${operation_args[@]}" "${identity_args[@]}" >/dev/null 2>&1; then
    return 1
  fi
