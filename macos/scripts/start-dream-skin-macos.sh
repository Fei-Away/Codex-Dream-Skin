...
if [ "$DEBUG_READY" = "false" ]; then
  ...
  CODEX_PID="$(codex_host_pid_for_port "$PORT")" \
    || fail "Could not bind the verified debug endpoint to its Codex host process."
fi

CODEX_STARTED_AT="$(process_started_at "$CODEX_PID")"
CODEX_EXECUTABLE="$(process_executable_path "$CODEX_PID")"
[ -n "$CODEX_STARTED_AT" ] && [ -n "$CODEX_EXECUTABLE" ] \
  && pid_is_codex_executable "$CODEX_PID" \
  || fail "Could not verify the Codex host PID, start time, and executable path."

if [ "$DEBUG_READY" = "false" ]; then
  # Start probing immediately instead of waiting for the native window to finish loading.
  if [ "$FOREGROUND_INJECTOR" != "true" ]; then
    INJECTOR_PID="$(launch_injector_daemon \
...
if [ "$FOREGROUND_INJECTOR" = "true" ]; then
  exec "$NODE" "$INJECTOR" --watch --port "$PORT" --theme-dir "$THEME_DIR" \
    ...
    --host-pid "$CODEX_PID" --host-started-at "$CODEX_STARTED_AT" \
    --host-executable "$CODEX_EXECUTABLE" --watcher-state "$STATE_PATH"
fi
...
write_state "$PORT" "$INJECTOR_PID" "$INJECTOR_STARTED_AT" \
  "$CODEX_PID" applying "$CODEX_STARTED_AT"
