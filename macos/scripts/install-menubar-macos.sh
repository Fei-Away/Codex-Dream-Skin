...
if [ -d "$INSTALL_ROOT" ] && [ "$PROJECT_ROOT" != "$INSTALL_ROOT" ]; then
  ...
  for name in pause-dream-skin-macos.sh status-dream-skin-macos.sh apply-from-menubar-macos.sh \
    switch-theme-macos.sh load-image-theme-macos.sh import-theme-package-macos.sh install-menubar-macos.sh; do
...
/bin/chmod 755 \
  ...
  "$PROJECT_ROOT/scripts/import-theme-package-macos.sh" \
