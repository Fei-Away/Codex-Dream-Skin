...
import { createMacosStudioAdapter } from "../platform/studio-adapter.mjs";
...
async function resolveSharedRoot() {
  ...
  for (const candidate of candidates) {
    try {
      await fs.access(path.join(candidate, "studio", "server.mjs"));
      return candidate;
    } catch {}
  }
...
const adapter = await createMacosStudioAdapter();
