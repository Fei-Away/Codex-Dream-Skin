...
- 宽幅画作通过 CSS anchor 自动以 Review / Terminal / Browser / Files 右侧面板的实时左边界作为画布右边界；拉伸底部面板导致右侧面板高度缩短时，画作不再被误判为面板关闭而重新缩放或裁切。实现保持统一 runtime 的纯属性 / CSS 约束，不引入 renderer 布局读取或业务 class。
- 任务页左侧栏使用与首页一致的半透明材质，避免进入编辑状态后背景颜色和饱和度明显下降。
