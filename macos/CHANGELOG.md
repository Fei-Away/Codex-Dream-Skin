...
- macOS 更新提醒支持经过验证的引导式下载：用户明确确认后，客户端从精确 tag 的 GitHub Release 下载 DMG 与 `SHA256SUMS.txt`，严格核对唯一资产、下载 URL、大小、GitHub API SHA-256、校验清单和 DMG 完整性；只读挂载后还会确认 App bundle ID、版本、内嵌引擎版本与代码签名。全部通过才打开 DMG，仍由用户手动拖入 Applications，不会静默自替换。
