...
- macOS 安装器现在通过原子、不跟随符号链接的帮助器写入桌面启动器；只有首两行严格匹配项目 shebang 与所有权标记的普通文件才允许替换。

### 修复

- `restore-dream-skin-macos.sh --uninstall` 会先验证全部四个桌面入口，再仅删除 Dream Skin 自己的启动器；用户后来放置的同名文件、目录或符号链接会被保留并安全拒绝。
