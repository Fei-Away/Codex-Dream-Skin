...
- 自动加载守护进程在本次 Codex 会话已经出现后尊重用户主动退出，只等待下次手动启动，不再立即重新打开 ChatGPT/Codex。

## 1.5.2 — 2026-07-17 (local Miku Dream extension)

### 新增

- 保留 macOS 用户级自动加载、LaunchAgent 监督器和 Theme Studio；安装默认启用，也可通过 `--no-auto-load` 或桌面入口关闭。
- 保留 `Miku Dream` 内置主题包、侧边栏配套样式，以及 Codex 26.707 新版 composer 结构标记。

### 修复

- 将消息输入栏底部原生深色渐隐替换为浅青白渐变，避免在 Miku Dream 背景上出现黑色阴影。
- 合并上游自适应图像主题和安全配置解析，同时继续保留原有 macOS 自动加载功能。
