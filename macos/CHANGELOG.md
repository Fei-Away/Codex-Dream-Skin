...
- 移除会覆盖原生标题栏的主题标语与定位规则，不再调整返回、前进、侧栏和窗口控件的原生布局，恢复左右工具栏的安全间距、点击区域与可读性。
- 将宽幅任务背景重构为单一画布：画作只存在于最底层，连续阅读遮罩在同一主平面过渡，不再创建独立的右侧图片窗口；输入框同时改为完整、独立的高层 surface，避免背景、遮罩、裁切或层级造成右端断裂。
- 补齐浅色主题对原生面板、会话摘要、输入、弹窗、提示状态和选中态的语义色映射，避免深色宿主 token 穿透后产生低对比度文字或表面。
- 注入验证现在会检查原生会话滚动容器仍可滚动；主题若覆盖其 `overflow`、裁切或交互能力，将安全失败而不是提交一个无法使用的活动状态。
