...
- 新增「Codex 2007 完整复古壳」预设：提供 macOS 兼容的固定标题栏、蓝白工具栏、三栏工作区、好友栏和复古输入框布局，同时保留原生 Codex 工作流。
- QQ2007 输入框的 A 按钮新增常用 Prompt 菜单，内置代码审查、修复 Bug、重构、测试、解释、PR 和截图检查模板，支持本地保存、编辑、删除和排序。
...
- 复古布局修复设置页返回入口被顶栏遮挡、左栏与主区间距过宽、右侧好友栏关闭状态、输入框基线/高度/长文本滚动，以及窄窗口文件预览挤压对话区等问题。
