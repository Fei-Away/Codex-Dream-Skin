...
- 新增「天空花园双姝」内置完整 UI 预设：包含冰蓝花园背景、侧栏主题卡片、导航角色图标、前景双角色、顶部趴卧角色、眨眼与摆腿、流动光影和 14 枚花瓣。
- 主题专属素材作为引擎内可信资源按精确 preset id 加载；普通预设和用户主题仍只使用既有 JSON 格式，不开放任意 CSS、JavaScript 或文件路径。

### 测试

- 覆盖角色舞台幂等创建、热关闭清理、重新打开、遮挡避让、后台暂停、图标装饰，以及主题专属 payload 不泄漏到其他预设。
