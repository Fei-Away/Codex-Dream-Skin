...
- 修复 GitHub 未登录 API 额度耗尽时「立即检查更新」直接报 HTTP 403 的问题。更新检查仍优先使用 Releases API；请求失败时改为读取固定官方 `releases/latest` 地址的跳转目标，并只接受当前仓库下通过严格语义版本校验的 tag，不需要用户配置 GitHub Token。
