...
- `customize-theme-macos.sh --reset-demo` 现在会把内置 `theme.json` 和背景图按“图片先、清单后”原子恢复到活动主题目录，不再只删目录后让后续启动因缺少主题而失败。

### 安全

- `write-theme.mjs reset-demo` 不再对任意项目外 `--output-dir` 执行递归删除。目标必须是名为 `theme` 的真实目录，项目路径、符号链接、普通文件和任意目录名均会被拒绝；恢复过程保留无关文件与旧图片。

### 测试

- 新增 reset-demo 项目路径、符号链接、任意目录拒绝，以及内置素材完整恢复和无关文件保留覆盖。
