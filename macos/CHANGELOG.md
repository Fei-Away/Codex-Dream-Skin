...
- Codex 自升级或从 Dock 重新打开后，会自动恢复已启用的主题
- 恢复 watcher 只接管已经运行但缺少主题端口的 Codex；用户主动退出后不会擅自拉起应用
- watcher 同时校验注入器存活状态，避免调试端口还在但刷新后主题丢失
- 一次性注入校验不再被异常的 CDP 关闭握手长期挂住
- 修复新版 Codex 进程识别与内置默认主题的状态显示
- 暂停会禁用重新打开恢复，完全恢复与卸载会移除对应 LaunchAgent
