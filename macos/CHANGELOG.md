...
- 增加只监听 `127.0.0.1` 的 Dream Skin Studio：控制台、24 套 Theme v3 官方场景、主题库、双调色板编辑器、三视图实时预览、WCAG 对比度提示和诊断页统一在浏览器中管理。
- Theme v3 场景层为新建任务页提供主题 Hero、四张行动卡、项目选择器衔接和动态场景角卡；行动卡子菜单真实转交 Codex 原生“添加 / 插件 / 智能体 / 项目”入口。
- 自定义主题支持创建、编辑、参考创建、应用与删除；官方场景支持直接应用或参考创建。

### 改进

- Studio、主题 schema、24 套主题资源和场景层由 `shared/` 复用；macOS injector 与基础样式继续保留平台实现，以维持官方签名 Node、图像分析和原生窗口适配。
- 移除新建任务 Hero 右上角信息价值有限的“场景状态”浮层，保留完整背景构图；旧主题中的 `widget` 数据仍可读取。
- 安装目录与发布 ZIP 现在携带完整共享资源，源码目录和安装后布局使用同一套 Studio 与场景能力。
- 菜单栏在 `paused` 状态显示「继续显示皮肤」（调用现有应用入口），与 Windows 托盘暂停/继续文案和语义对齐；运行中仍为「重新应用皮肤」+「暂停皮肤」。

### 安全

- Studio 的开始、恢复和重新应用继续使用现有重启确认、主题原子暂存、图片大小与像素限制、CDP 端点归属和进程身份校验；不会静默改写官方应用或 `app.asar`。

### 修复

- 修复行动卡子菜单被层叠区域拦截、鼠标释放后立即关闭原生菜单，以及工具、技能和项目入口只显示装饰而不能操作的问题。
- 修复 Theme v3 首页在 Codex 新版 DOM 下标题重叠、行动卡缺失、Composer 与项目栏挤压和窄窗横向溢出的问题。
