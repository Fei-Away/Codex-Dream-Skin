...
- 修复重新应用时取消官方退出确认仍被强制结束并重启的问题（#398）。热应用失败后另行请求重启确认；取消终止操作，官方退出超时或仍在运行时不再发送 TERM/KILL，也不把取消显示为应用成功。
- 修复 GitHub 未登录 API 额度耗尽时「立即检查更新」直接报 HTTP 403 的问题（#401，感谢 @guoqunabc）。更新检查仍优先使用 Releases API；请求失败时改为读取固定官方 `releases/latest` 地址的跳转目标，并只接受当前仓库下通过严格语义版本校验的 tag，不需要用户配置 GitHub Token。跳转头解析兼容 macOS BSD awk，限制头大小，并仅采用最终响应中的唯一跳转头。
