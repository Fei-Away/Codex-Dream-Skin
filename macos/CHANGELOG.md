...
- 浅色任务页的左右侧栏改为主题色驱动的海盐玻璃层，避免不透明白色面板割裂背景
- 选中项目使用主题强调色，右侧环境摘要卡片与左侧导航统一磨砂、描边和阴影

### 性能

- 侧栏不再重复绘制背景图或使用整列实时模糊，任务页停止装饰粒子动画
- 降低摘要卡片、正文卡片与输入框的模糊半径，顶栏改用静态透明层
- DOM 观察器忽略无关属性变化，主题变量与装饰内容只在实际变化时写入
