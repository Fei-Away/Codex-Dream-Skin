...
- 修复 macOS 菜单栏「重新应用皮肤」与「打开 ChatGPT」的入口语义不一致问题（#341）。「重新应用皮肤」现在先无弹窗热重载，只有热重载不可用时才提示打开或重启 ChatGPT；「打开 ChatGPT」保留原菜单标题，但改走 Dream Skin 启动/应用链路，不再只是原生打开官方 `ChatGPT.app`。
