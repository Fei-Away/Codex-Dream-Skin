...
- 原生 macOS SwiftUI 可视化管理器：选图、三套配色、示例主题、检查截图、恢复和卸载
- GUI 内置当前仓库引擎，用户无需另外安装 Node.js
- 运行状态、进度遮罩、操作日志与危险操作确认
- `.app` 与 DMG 构建脚本，以及无警告 Swift 类型检查

### 改进

- 恢复与卸载支持重复执行，已经处于官方外观时不会再次要求缺失的主题备份
- GUI 使用独立临时日志文件收集脚本输出，避免 Codex 重启继承输出管道后界面无限等待
- GUI 操作加入 75 秒硬超时，并在重启阶段显示预计等待时间
