...
- [next] Task 9（check-update 移植、community-http.mjs/community-apply.mjs/community-apply-linux.sh 一键换肤、install 移植）。计划代码坑已记录：(1) community-http.mjs 零重定向实现有洞（redirect 分支只查 <0，首个重定向被跟随）；(2) oversize-header 测试夹具算术错（32/32 不触发预检）；(3) community-apply-linux.sh 的 import --expected-sha256/--expected-bytes 与 status --deep 参数需按实际脚本核对。
