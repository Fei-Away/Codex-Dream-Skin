...
- [next] Task 11（build-tarball.sh + build-release-linux.sh：sync --check 闸门 + 测试 + 双包 + SHA256SUMS）。关键：必须修 runtime/theme-package-validator.mjs（allowed set 加 linux、max 2→3）再 sync 重新生成三端资产，否则 sync 会洗掉 Task 7 手补的 linux 支持；顺带修 build-deb.sh 的 printf。
