...
- [next] Task 10（.deb 打包：installer/ 素材 + build-deb.sh + deb-content 断言 + run-tests.sh 覆盖 installer 无扩展名脚本与 node>=18）。计划坑已记录：build-deb 必须把 usr/bin/dreamskin 软链打进包内（断言要求）；control 需补 ffmpeg（Recommends）；Step 5 版本路径从 VERSION 派生。
