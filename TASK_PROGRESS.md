...
## Pet overlay + Dream Skin compatibility — PR open and synced to Metis (2026-08-04)
...
- [root cause] The native Pet activity pill intentionally uses a transparent material. With Dream Skin painting a high-contrast wallpaper behind the separate Pet composition windows, each `activity-slot-0` through `activity-slot-3` surface samples the wallpaper into a rounded rectangle. Separately, the Pet avatar root can expose enough generic shell markers to pass the old main-renderer probe, so an operation-progress host could be shown there without receiving the matching completion event. During an upgrade, the old watcher could also inject into another Pet auxiliary surface immediately before it stopped, leaving a stale loading host for the new, correctly bounded watcher to ignore.
- [implemented] The watcher recognizes only the exact four activity-slot pages, re-verifies live identity, and applies an opaque native-token background only to `[class*="_activityPillMaterial_"]`. Main-renderer discovery now requires the controlled `app://-/index.html` shape and excludes `initialRoute=/avatar-overlay` before connecting or probing. On startup, exact known Pet auxiliary targets are live-reverified once and only Dream Skin's own operation host/registry is cleared; unknown or ambiguous targets fail closed. Main Codex, avatar root, mascot, voice output, voice controls, and microphone remain free of theme CSS. Pause/shutdown removal is verified before acknowledgement; target replacement and same-target reload are reprotected.
- [tested] Each new contract first failed before implementation. The final complete `bash macos/tests/run-tests.sh` passes after the auxiliary-cleanup change, including Swift build/XCTest, shared runtime checks, Safe CSS, import/rollback, signed-runtime integration, runtime-state integration, and Doctor; `node --check` and `git diff --check` also pass. A real begin/finish operation test placed the exact token only in the main renderer and left zero hosts after completion. A real old-to-new watcher migration test seeded a stale Dream Skin host in `voice-output`; the new watcher removed it without touching Pet UI.
- [deployed locally] The installed v1.5.11 injector is byte-identical to implementation commit `8f4328b` (`cd0bdd76...`). Codex PID stayed `50570`; only the watcher changed to PID `77286`. All four activity slots contain the bounded style and compute to opaque `rgb(24, 24, 24)` while their HTML/body remain transparent. Main/avatar/mascot/voice surfaces contain no compatibility style or operation host. Reloading activity slot 0 produced a replacement target that the watcher automatically protected again.
- [synced Metis] Metis fetched the PR branch into independent ref `refs/remotes/codex-sync/fix-pet-overlay-compat`; its dirty `main` and unrelated 92-line `TASK_PROGRESS.md` addition were preserved. The v1.5.11 installed injector is byte-identical to `8f4328b`, original and intermediate rollback copies remain available, Codex PID stayed `2020`, and only the watcher changed to PID `66502`. The seeded stale `voice-output` host was removed, main Dream Skin remained active, and all present non-activity Pet surfaces remained unstyled. Metis had no live activity slots during acceptance, so four-slot rendering evidence comes from the same exact code on the local Mac.
- [accepted locally] The user confirmed the native composite returned to normal after the task-state spinner ended.
- [GitHub] Draft PR #339 (`semantic-craft:codex/fix-pet-overlay-compat` -> `Fei-Away:main`) contains implementation commits `52081be`, `c4e9bc3`, and `8f4328b`; the PR description records both root causes, impact, and validation.
- [pending] PR review and CI. No merge, version bump, tag, or Release has occurred.
