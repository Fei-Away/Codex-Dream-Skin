...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-dream-skin.ps1` once. The installer preserves the user's native appearance settings, seeds the OpenAI China theme, copies the runtime to `%LOCALAPPDATA%\CodexDreamSkin\engine`, and creates launch/restore/tray shortcuts that do not depend on the source checkout. Passing `-AutoApply` also adds the tray shortcut to the current user's Startup folder.
2. Use the `Codex Dream Skin` shortcut or run `%LOCALAPPDATA%\CodexDreamSkin\engine\scripts\start-dream-skin.ps1`. If `-AutoApply` was enabled at install time, the tray checks for a normal Codex main window every 250ms. A normal launch has no usable CDP port, so the tray reopens it with Dream Skin arguments by calling the launcher with `-RestartExisting -FastRestart`. CLI callers must explicitly add `-RestartExisting`.
...
- Auto-apply must be install-gated, fast, and loop-safe. It should act only on a normal Codex main window without a verified Dream Skin CDP identity, use the saved or next free loopback port, and pause with a tray notification after repeated startup failure or timeout instead of cycling forever.
...
- Treat `%LOCALAPPDATA%\CodexDreamSkin\engine` as an installer-managed runtime. Exit the Dream Skin tray before reinstalling so the installer can replace that runtime atomically and update every shortcut to the same copy, including the optional Startup tray shortcut.
...
- `assets/openai-china-hero.png`: OpenAI China home-banner artwork seeded as the Windows default theme.
- `assets/dream-reference.jpg`: legacy sample wallpaper retained for custom-theme import and tests.
