...
1. Install Node.js 22 or newer, close Codex, then run `scripts/install-dream-skin.ps1` once. The installer preserves the user's native appearance settings, seeds the Arina Hashimoto theme, copies the runtime to `%LOCALAPPDATA%\CodexDreamSkin\engine`, and creates launch/restore shortcuts that do not depend on the source checkout. Pass `-TrayShortcut` or `-StartTray` only when the user explicitly wants tray controls.
...
- Do not create a hidden PowerShell tray shortcut or start the tray by default. Tray controls must be explicit opt-in via `-TrayShortcut` or `-StartTray`.
