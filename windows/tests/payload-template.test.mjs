...
const temporaryRoot = await fs.mkdtemp(path.join(os.tmpdir(), "dream-skin-payload-windows-"));
const temporaryTheme = path.join(temporaryRoot, "theme");
const specialName = [
  "My $' Theme | Cool $& Skin | Price $$ Theme | Backtick $` Theme",
  "__DREAM_ART_JSON__",
...
try {
  ...
  const missingInjector = await createInjectorFixture(
    "missing-token",
    template.replace("__DREAM_ART_JSON__", ""),
  );
  ...
  assert.match(
    ...
    /must contain exactly one __DREAM_ART_JSON__; found 0/,
  );
  ...
  assert.match(
    ...
    /must contain exactly one __DREAM_THEME_JSON__; found 2/,
  );
} finally {
  await fs.rm(temporaryRoot, { recursive: true, force: true });
}

console.log("PASS: Windows payload safely replaces exactly one of every template token.");
