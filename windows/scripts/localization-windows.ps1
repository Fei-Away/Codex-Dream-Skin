...
function Get-DreamSkinText {
  ...
  $catalog = @{
    'en-US' = @{
      ...
      RecoverCannotRestart = 'Existing-session recovery cannot restart Codex or prompt for restart consent.'
      RecoverPaused = 'Injector recovery stopped because Dream Skin is paused.'
      RecoverNoState = 'Injector recovery stopped because no complete previous Windows session state is available.'
      RecoverWatcherRunning = 'Injector recovery stopped because the recorded watcher is already running.'
      RecoverSameSession = 'Injector recovery stopped because the verified Codex Browser identity has not changed.'
      RecoverPendingAppearance = 'Injector recovery stopped because an interrupted appearance update still needs reconciliation.'
      RecoverNoSession = 'No verified existing Codex debugging session is available for injector recovery.'
      RecoverSessionClosed = 'The verified Codex debugging session closed before injector recovery completed.'
    ...
    'zh-CN' = @{
      ...
      RecoverCannotRestart = '现有会话恢复不能重启 Codex，也不能请求重启授权。'
      RecoverPaused = 'Dream Skin 已暂停，注入器恢复已停止。'
      RecoverNoState = '没有完整的上一次 Windows 会话状态，注入器恢复已停止。'
      RecoverWatcherRunning = '记录的 watcher 仍在运行，注入器恢复已停止。'
      RecoverSameSession = '已验证的 Codex Browser identity 尚未变化，注入器恢复已停止。'
      RecoverPendingAppearance = '仍有中断的外观更新需要协调，注入器恢复已停止。'
      RecoverNoSession = '没有可供注入器恢复使用且已验证的现有 Codex 调试会话。'
      RecoverSessionClosed = '已验证的 Codex 调试会话在注入器恢复完成前关闭。'
