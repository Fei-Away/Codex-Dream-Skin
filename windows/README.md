...
当前 fork 版本为 `v1.5.9`，基于上游 `v1.5.6` 继续维护。原有的 Windows PowerShell 5.1 托盘菜单兼容修复已被上游吸收；左侧栏与菜单栏左段采用随侧栏宽度同步的连续深色背景块：首页右侧保持无遮罩，对话页菜单栏右段与主窗口遮罩连续，并适配 Codex 26.721+ 首页结构。v1.5.9 在新结构上恢复 v1.5.6/v1.5.7 的主题标题与玻璃建议卡视觉。
