...
在主题的 `theme.json` 中加入以下字段，即可启用精简的右侧任务工具栏：
...
修改后重新应用主题。工具栏包含带计时器的专注模式、原生侧栏开关、当前对话/上一轮定位和最新消息快捷操作。它只使用 Codex 的语义化侧栏与任务滚动元素；没有可滚动任务时会禁用对应按钮，并遵循系统的“减少动态效果”设置。暂停或恢复原生界面时，工具栏会被完整清理。
