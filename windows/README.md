...
- 从 Microsoft Store 安装且已注册到当前用户的官方 `OpenAI.Codex` 或 `OpenAI.CodexBeta` 应用。
- Node.js 22 或更高版本，`node.exe` 可从 `PATH` 找到。
- PowerShell 7，`pwsh.exe` 可从标准安装目录或 `PATH` 找到。图形控制面板不兼容 Windows PowerShell 5.1。
- Windows .NET Framework 4.x，用于在安装时从源码生成本机 GUI 启动器。
...
安装器会校验官方 Codex Store 包和 Node.js，保存可恢复的外观配置，初始化本地主题仓库，并从仓库中的 C# 源码生成 GUI 启动器。默认只创建一个桌面和开始菜单快捷方式：

- `Codex Dream Skin Console`：用于启动、主题切换、图片导入、暂停/继续、恢复和验证的图形控制面板。

控制面板会在需要时启动现有的托盘服务，但不会再为托盘和恢复功能分别创建桌面快捷方式。
...
推荐从 `Codex Dream Skin Console` 快捷方式打开控制面板。点击“应用”时，如果 Codex 已经运行，控制面板会先询问是否重启。
...
打开 `Codex Dream Skin Console` 后可以：
...
图形控制面板必须使用 PowerShell 7，不能回退到 Windows PowerShell 5.1。安装 PowerShell 7 后运行 `pwsh --version`，确认命令可用，再重新打开控制面板。
...
通过 `Codex Dream Skin Console` 应用皮肤后再运行验证。普通 Codex 启动方式不会打开 Dream Skin 所需的调试会话。
