...
在主题的 `theme.json` 中加入以下字段，即可在任务输入框下方启用细额度条：
...
修改后重新应用主题。额度条只读取当前任务本地 Codex JSONL 会话日志中的 `token_count` 事件；字段可用时显示主额度窗口剩余量、上下文窗口用量和重置日期。缺少数据时明确显示不可用，不做猜测。首次有界尾读后会缓存任务文件并只读取新增字节，不扫描 React 内部对象、账户菜单、消息正文或网络流量。ChatGPT 页面不显示该额度条。
