...
安装器会校验官方 Codex Store 包和 Node.js，保存可恢复的外观配置，并初始化本地主题仓库。默认会创建这些快捷方式：

- `Codex Dream Skin`：启动或重新应用皮肤。
- `Codex Dream Skin - Restore`：恢复官方外观并关闭已保存的 CDP 会话。

如需同时创建系统托盘主题控制，再显式增加 `-TrayShortcut`。如果 ZIP 下载方式给脚本留下 Internet-zone 标记，请先对这份源码运行 `Unblock-File`，再使用 `RemoteSigned` 安装。安装器会先校验运行时副本的 SHA-256，再仅对 `%LOCALAPPDATA%\CodexDreamSkin\engine` 中受管的 PowerShell 副本清除下载区标记。日常快捷方式使用 `RemoteSigned`，不会绕过系统或企业组策略。
...
安装时传入 `-TrayShortcut` 后，打开 `Codex Dream Skin - Tray` 可以：
...
旧版托盘快捷方式同时使用隐藏 PowerShell 和 `ExecutionPolicy Bypass`，可能触发基于行为特征的 LNK 告警。不要直接加入白名单；更新源码并重新运行安装器。新版默认不再创建托盘快捷方式；只有显式传入 `-TrayShortcut` 时才会安装托盘入口，且快捷方式使用 `RemoteSigned`。如果新版仍然报警，请保留隔离状态，并在 Issue 中附上杀毒软件名称、版本、告警名称和快捷方式属性，不要上传密钥或私人数据。
