...
- 新增无第三方依赖的 Windows Split Theme Studio：左侧任务栏与右侧工作区拥有独立拖放/预览区域；两张 PNG、JPEG 或 WebP 会分别经过现有格式和尺寸校验，并受 16 MB 合计预算约束。应用、保存和切换主题会保持双图自包含；旧主题未声明 `sidebarImage` 时继续使用单图整窗布局。安装器会在桌面和开始菜单创建独立入口，系统托盘的换图操作也会打开该界面。
