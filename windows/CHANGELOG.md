...
- 从系统托盘切换「已保存主题」后会立即调用启动脚本重新应用皮肤；当 watcher 已退出或 Codex 是普通方式启动时，会走现有确认重启流程，而不是只更新主题文件。
- 哥特宽图主题的 Windows 首页不再额外压暗整块原生 header，让顶部背景分界与会话页保持一致。
- 暗色弹窗里的非主按钮默认态改用暗底与暗金边，避免重命名任务等确认弹窗的「取消」按钮出现亮白底色。
