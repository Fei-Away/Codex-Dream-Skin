...
- 系统托盘新增普通 `.zip` 主题包导入与主题目录快捷入口；支持带完整 manifest 校验的正式 Studio 包，以及仅供可信本地内容使用的两文件简化包。导入只写入已保存主题，不自动改变活动主题。基于 .NET `ZipArchive` 逐条受限解压，限制 32 MiB 压缩文件、32 个条目和 64 MiB 解压总量，并拒绝 `.dreamskin`、路径穿越、链接/reparse、Windows 保留路径、嵌套压缩包及未通过负载校验的内容。正式包会校验平台、最低客户端版本、逐文件大小与 SHA-256；`theme.css` 保留但不执行，预留签名不验证。重复内容不再写入，同 ID 冲突自动保存为新标识；同时保留手动移动已解压主题目录的工作流。
