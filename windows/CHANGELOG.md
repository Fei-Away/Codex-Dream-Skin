...
- 与 macOS 共用可版本化的 `theme.json` v1 契约和有效/无效主题夹具；主题包现在可使用安全的子目录图片、便携 `palette` 颜色令牌以及 32 KB 反向域名 `extensions` 数据命名空间。新建和保存的 Windows 主题会显式写入 v1，旧版无版本主题保持兼容，未来或错误版本会明确拒绝。
