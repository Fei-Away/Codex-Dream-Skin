...
- 支持将本机 MP4 设为动态视频背景：托盘新增「一键更换视频背景」，主题包 `theme.json` 可选 `video` 字段；视频经本机回环媒体服务传输（`127.0.0.1`、一次性 token、Range/`206`），图片仍作 poster 与失败回退。导入校验要求真实 `ftyp` 容器头、非空且不超过 100 MiB；热切换仅在渲染器确认可播放后提交，失败则恢复切换前主题。
