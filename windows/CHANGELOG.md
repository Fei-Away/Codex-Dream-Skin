...
- 新增离线 `.dreamskin` v1 导入：托盘和 PowerShell 入口会先流式校验包、哈希、图片与 Windows payload，再原子安装到主题库；支持摘要确认、幂等重复导入、显式替换和安装后立即/稍后应用。
- Windows 受管 runtime 会一并安装共享主题包核心、Schema、示例和 Kimi 作者文档，源码目录删除后仍可导入。

### 修复

- 载荷模板改为单次、计数校验的占位符替换；主题文本中的 `$&`、`$'`、`$$`、“美元符号 + 反引号”和形似内部占位符的普通文字不再破坏注入脚本。
