...
- `--verify` 现在分别报告主区域与首页层的验证状态；非首页会把首页标为 `skipped`，主区域标记缺失时不再返回 `pass: true`。
- 停止已记录注入器时会在重新核验 PID 身份后重试并延长轮询等待；恢复流程不再因单次 5 秒等待超时直接中断。
- Windows 导入非 16:9 或低于建议 2560 × 1440 的图片时会提示可能裁边或放大变糊，托盘通知同步展示提示且不会静默改写原图。
