...
- 修复完全关闭并重新打开 Codex 后 watcher 随旧 Browser identity 退出、托盘换肤只写入文件却不再更新界面的问题。长驻托盘现在只会在旧 watcher 的 PID 与启动时间已失效、不同 Browser ID 的新端点再次通过官方 Store 包所有权校验后恢复一个 watcher；暂停、端口关闭、非官方 listener 或待协调外观事务均保持 fail-closed，恢复路径不会启动或重启 Codex（#200、#231）。
