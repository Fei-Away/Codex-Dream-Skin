...
- 主题可通过 `features.usageMeter: true` 启用本地隐私安全额度条，仅增量读取当前任务 JSONL 中的 `token_count` 事件，显示主额度窗口、上下文用量与重置日期；缺失字段保持不可用且不扫描 React 内部对象（#159）。
...
- 额度条通过 `ResizeObserver` 跟随输入框尺寸与位置变化，并以单一合成位移实时更新；左右侧面板展开或收起时会保持与输入框等宽居中，不再在旧位置留下残影。
