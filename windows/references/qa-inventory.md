...
- Shortcut policy: installed launch, restore, tray, and tray-child commands use `RemoteSigned` without `Bypass`; Internet-zone markers are removed only from hash-verified managed PowerShell copies. A default install must not create a tray shortcut or start a hidden tray process; tray controls require explicit opt-in.
