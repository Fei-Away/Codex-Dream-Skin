...
function makeFixture({ nativeAppearance = "dark", settings = false, adopted = true } = {}) {
  ...
  if (!settings) {
    ...
    register("main[data-app-shell-main-surface]", partFixtures.main);
    register("header[data-app-shell-application-menu-bar]", partFixtures.header);
    register('[role="main"] h1.heading-xl', partFixtures.homeIcon);
    ...
    register('[data-turn-key]', partFixtures.message);
  ...
  const addDynamicMessage = () => {
    const node = makeDomNode(`message-${(selectorNodes.get('[data-turn-key]') || []).length + 1}`, partFixtures.thread || body);
    register('[data-turn-key]', node);
    return node;
  };
  return {
...
export async function runRendererRuntimeTest(assetRoot) {
  ...
  assert.match(css, /main\[data-app-shell-main-surface\]:has\(\[role="main"\]\)/);
  assert.match(css, /main\[data-app-shell-main-surface\]:not\(:has\(\[role="main"\]\)\)/);
