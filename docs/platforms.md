...
同一套主题配置在两个渲染器中使用相同的外观约定。机器可读的 v1 核心规范见 [`schemas/theme-v1.schema.json`](../schemas/theme-v1.schema.json)，完整制作说明见 [`portable-theme-authoring.md`](./portable-theme-authoring.md)。只需要提供一张背景图，注入层就会在本机分析图像并生成协调的颜色、焦点和页面布局；不会上传图片，也不依赖外部 AI/API。
...
- `schemaVersion`：发布的便携主题显式使用 `1`；两端为兼容已有本地主题，会把缺省版本视为旧版 v1，但拒绝显式的未知版本或字符串版本。
- `image`：主题包内的相对路径，允许 `images/background.jpg` 等子目录；规范化路径以及链接/联接目标都必须留在主题目录内。
...
- `palette`：跨平台的 `accent`、`background`、`surface`、`surfaceAlt`、`text`、`muted`、`line` 颜色令牌。macOS 原有 `colors` 字段在冲突时优先；Windows 映射到原生 `--dream-*` 变量。
- `extensions`：反向域名命名空间的纯 JSON 扩展数据，总计不超过 32 KB。两端把它暴露在 renderer 状态中但不会作为代码执行；未知顶层字段继续允许并忽略，以便工具和未来版本渐进扩展。
