...
macOS 菜单栏与 Windows 系统托盘中的已保存主题使用可撤销的试穿事务，不再在点击后直接永久替换当前主题：

1. 先把活动主题和候选主题分别复制为自包含快照，并完成平台现有的路径、图片与暂存后完整 payload 校验。
2. 快照完整后才创建 `theme-preview` 待确认标记；随后以图片先行、`theme.json` 最后提交的顺序发布候选主题。
3. 用户选择保留时，先原子移走待确认标记，再清理快照；选择恢复时，先从备份恢复活动主题，成功后才清理标记。
4. 试穿进程异常退出时，下一次启动或菜单刷新会同时核对 PID 和进程启动时间。只有确认原进程已经消失，才自动恢复试穿前主题；PID 被系统复用时不会误判为原试穿进程。

Windows 需要 Dream Skin 正在运行且未暂停，托盘才会打开实时试穿确认框；否则会先提示用户执行「应用或重新应用」。macOS 会复用已验证的本机 CDP 热重载，必要时走现有启动流程。两端确认框都把「恢复原主题」作为默认选择，降低误保留风险。两端也都只操作各自用户状态目录，不修改官方 Codex 应用、`app.asar`、签名、API Key 或 Base URL。
