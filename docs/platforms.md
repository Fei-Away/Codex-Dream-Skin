...
同一套主题配置在两个渲染器中使用相同的外观约定。默认只需要提供一张背景图，注入层就会在本机分析图像并生成协调的颜色、焦点和页面布局；Windows 还可提供可选的 `sidebarImage`，把左侧任务栏与右侧工作区分成两张独立图片。图片不会上传，也不依赖外部 AI/API。
...
- `image`：主图片；Windows 双分区模式把它限制在右侧工作区。`sidebarImage` 是 Windows 的可选相对路径；存在时左侧原生 `aside.app-shell-left-panel` 单独绘制该图，不存在时保留兼容的单图整窗模式。
...
- Windows 安装会把运行所需的 `assets/` 与 `scripts/` 原子复制到 `%LOCALAPPDATA%\CodexDreamSkin\engine`，所有快捷方式均指向该受管副本，因此安装后可移动或删除源码目录。安装还会初始化 `active-theme`、`themes` 与 `images`，并把「桥本有菜」同时设为首次默认和可切换的已保存主题。独立 Theme Studio 为左侧任务栏与右侧工作区提供两个拖放/预览区域，并显示尺寸和体积；应用、保存与切换都会保持两张图片自包含。系统托盘保留切换已保存主题、暂停和恢复，并可打开 Theme Studio。图片与 `theme.json` 保存在主题仓库中，不写进 Codex 的 `config.toml`，也不会上传网络。安装会保留用户已有的 `appearanceTheme`；仅在识别到旧版精确托管的浅色三元组时按备份迁移。
...
- Windows 导入和 macOS 快速加载入口会拒绝空文件或超过 16 MB 的输入；Windows 双分区主题的两张图片合计仍不得超过 16 MB。macOS 主定制入口可接收最高 50 MB 的源图，但转换后的主题文件必须不超过 16 MB。两端 payload 构建还会拒绝任一边超过 16384px 或总像素超过 50MP 的声明尺寸；Windows 在复制每张导入图前复用 Node 元数据解析器执行同一限制。Windows 注入器把主图、侧栏图与主题内容一起纳入 SHA-256 修订值，并在构建首帧 payload 前同步读取两张图的元数据。
