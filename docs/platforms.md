...
| 一键恢复 | ✅ | ✅ |
| 实机 verify / 截图 | ✅ | ✅ |
| 隐私安全 Support Snapshot | ✅（只读 JSON） | ✅（只读 JSON） |
| 用户选图定制 | ✅ | ✅（系统托盘「更换背景图」） |
| 本地主题保存 / 切换 | ✅（菜单栏） | ✅（系统托盘） |
...
## 隐私安全 Support Snapshot

排障前可先生成一份只读 JSON，并在自行检查后手动决定是否附到 Issue：
...
```powershell
# Windows（安装后的受管运行时）
powershell -NoProfile -File "$env:LOCALAPPDATA\CodexDreamSkin\engine\scripts\support-snapshot-windows.ps1"
```

该命令不写文件、不连 CDP、不访问网络，也不自动上传任何内容。输出仅保留版本与状态枚举；会明确排除路径、端口、PID、Browser ID、日志、截图、主题元数据、配置内容、环境变量、凭据和聊天/任务内容。需要实时界面或 CDP 证据时，仍使用现有 Verify / Doctor 流程。
