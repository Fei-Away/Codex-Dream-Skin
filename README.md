...
macOS 与 Windows 现在可以通过 `Dream Skin Studio` 管理主题：

- 浏览并直接应用 24 套 Dream Skin 官方场景和本机自定义主题。
- 编辑浅色/深色调色板、背景焦点、遮罩、面板、场景身份和四张行动卡。
- 在首页、任务和差异视图中实时预览，并检查正文 `4.5:1`、控件 `3:1` 的 WCAG 对比度。
- 查看 Codex、CDP、注入器、当前主题、路径和日志状态，执行开始、暂停、重新应用与恢复。

控制台仅监听 `127.0.0.1` 并使用本地令牌。24 套“官方场景”由 Dream Skin 项目维护，不代表 OpenAI 官方主题；历史图库仍是设计参考，不会作为背景直接注入。
