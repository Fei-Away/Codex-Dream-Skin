...
- **可恢复**：一键还原官方外观
- **可自动加载**：macOS 可随用户会话自动启动 Codex 并持续加载主题，也可随时关闭
- **可自动更新加载**：主题图片、配置、样式或注入脚本变化后，当前 Codex 会自动重新载入最新版本
- **可视化定制**：macOS Theme Studio 可调整背景图、外观模式、文案和配色
- **内置主题包**：macOS 安装后可直接切换 `Miku Dream` 粉青紫浅色主题
